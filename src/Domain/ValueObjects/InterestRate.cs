...
public record InterestRate
{
    ...
    public static InterestRate Zero(InterestRateType type = InterestRateType.Annual) => new(0, type);

    public static InterestRate operator +(InterestRate left, InterestRate right)
    {
        if (left.Type != right.Type)
            throw new InvalidOperationException($"Cannot add interest rates with different types: {left.Type} and {right.Type}");
    ...
    {
        if (left.Type != right.Type)
            throw new InvalidOperationException($"Cannot subtract interest rates with different types: {left.Type} and {right.Type}");
